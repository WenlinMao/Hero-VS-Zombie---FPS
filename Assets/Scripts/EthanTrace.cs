...
public class EthanTrace : MonoBehaviour {
	...
	void Start () {
		...
		target = GameObject.Find("FPSPlayer"); // set target to player
		timer = 0.0f;
	}
	...
	void Update () {
		...
		if (GameManager.gm==null || GameManager.gm.gameState == GameManager.GameState.Playing) {
			...
			if (dist > minDist && dist < maxDist) {	//当敌人与目标的距离大于追踪距离，追踪目标
			...
			// 当敌人位于最大追踪距离之外，追踪并进行闪躲
			else if (dist >= maxDist){
				// When timer greater than changeTime, change direction randomly
				if (timer >= changeTime) {
					timer = 0.0f;
					transform.LookAt (target.transform);
					Vector3 euler = transform.eulerAngles;
					euler.y = Random.Range (euler.y - deltaRotate, euler.y + deltaRotate);
					transform.eulerAngles = euler;
				}
				// else cumlatively add time
				else {
